...
const OBJECT_STORE_NAME = "shapes";
const SNAPSHOT_KEY = "snapshot";
const UPDATE_KEY_PREFIX = "update-";
const METADATA_KEY = "metadata";
const CONSOLIDATION_THRESHOLD = 10; // Consolidate after N updates
...
export class LocalDB {
  ...
  public async initialize(): Promise<void> {
    this.db = await openDB(this.persistenceKey, 1, {
      upgrade(db) {
  ...
  public saveDoc = throttle(async (doc: LoroDoc): Promise<void> => {
    if (!this.db) {
      throw new Error(
